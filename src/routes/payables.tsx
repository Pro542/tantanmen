...
import { Search, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, AlertCircle, Clock, CheckCircle2 } from 'lucide-react'
import type { Payable, PayableStatus } from '#/data/finance'
import { payables, formatCurrency } from '#/data/finance'
...
export const Route = createFileRoute('/payables')({
  validateSearch: (search: Record<string, unknown>): PayablesSearch => ({
    ...
    sortBy: (['vendor', 'amount', 'dueDate', 'status'].includes(search.sortBy as string)
...
const statusConfig: Record<PayableStatus, { label: string; cls: string; icon: typeof Clock }> = {
...
function PayablesPage() {
  ...
  const filtered = payables.filter(
    (p) =>
      ...
      (query === '' || p.vendor.toLowerCase().includes(query.toLowerCase()) || p.invoiceNo.toLowerCase().includes(query.toLowerCase())),
  )
  ...
  return (
    <div className="flex flex-col h-full overflow-auto">
      ...
      <header className="px-8 py-5 bg-white border-b border-slate-200 shrink-0">
        ...
        <p className="text-sm text-slate-500 mt-0.5">Manage vendor invoices and payments</p>
      </header>
      ...
      <div className="flex-1 p-8 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Total Outstanding', value: formatCurrency(outstanding), cls: 'border-slate-200' },
            { label: 'Overdue', value: formatCurrency(overdue), cls: 'border-rose-200 bg-rose-50' },
            { label: 'Due Soon (Pending)', value: formatCurrency(dueSoon), cls: 'border-amber-200 bg-amber-50' },
          ].map(({ label, value, cls }) => (
              ...
              {tabs.map((t) => (
                ...
                >
                  ...
                  <span className="ml-1.5 text-[10px] text-slate-400">
                    {t.key === 'all' ? payables.length : payables.filter((p) => p.status === t.key).length}
                  </span>
                </button>
              ))}
            </div>
                ...
                {pageItems.length === 0 ? (
                  ...
                  pageItems.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-3 text-xs text-slate-500 font-mono">{p.invoiceNo}</td>
