...
export function AreaChart({ data }: AreaChartProps) {
	...
	const yPos = (v: number) =>
		PAD.top + CHART_H - ((v - minVal) / range) * CHART_H;
	...
	const areaPath =
		`M ${firstX} ${PAD.top + CHART_H} ` +
		points.map((p) => `L ${p.x} ${p.y}`).join(" ") +
		` L ${lastX} ${PAD.top + CHART_H} Z`;

	const ticks = Array.from(
		{ length: TICK_COUNT + 1 },
		(_, i) => minVal + (range / TICK_COUNT) * (TICK_COUNT - i),
	);
	...
	return (
		<svg
			...
			aria-label="Cash position area chart"
		>
			<defs>
