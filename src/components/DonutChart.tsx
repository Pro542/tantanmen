...
export function DonutChart({ data }: DonutChartProps) {
  ...
  return (
    <div className="flex items-center gap-8">
      <svg viewBox="0 0 200 200" className="w-48 h-48 shrink-0" aria-label="Expense breakdown donut chart">
        {segments.map((seg) => (
          <path key={seg.category} d={annularSectorPath(seg.startDeg, seg.endDeg)} fill={seg.color} />
        ))}
        <text x={CX} y={CY - 6} textAnchor="middle" fontSize="9" fill="#94a3b8" fontWeight="500">
          TOTAL
        </text>
        <text x={CX} y={CY + 8} textAnchor="middle" fontSize="12" fill="#1e293b" fontWeight="700">
      ...
      <div className="flex flex-col gap-2.5 flex-1">
        {data.map((d) => (
          <div key={d.category} className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-800">{formatCurrency(d.amount)}</span>
