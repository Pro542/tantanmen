...
export function LineChart({ data }: LineChartProps) {
	...
	const outflowPoints = data.map((d, i) => ({
		x: xPos(i),
		y: yPos(d.outflow),
	}));
	...
	const toAreaPath = (pts: Array<{ x: number; y: number }>, color: string) => {
		...
		const firstX = pts.at(0)?.x ?? 0;
		const lastX = pts.at(-1)?.x ?? 0;
		return {
			d: `${line} L ${lastX} ${bottom} L ${firstX} ${bottom} Z`,
			color,
		};
	};
	...
	const ticks = Array.from(
		{ length: TICK_COUNT + 1 },
		(_, i) => (maxVal / TICK_COUNT) * (TICK_COUNT - i),
	);
	...
	return (
		<svg
			...
			aria-label="Revenue vs expenses line chart"
		>
			<defs>
			...
			<g transform={`translate(${W - PAD.right - 150}, ${PAD.top})`}>
				<line
					...
					strokeLinecap="round"
				/>
				<text x="20" y="9" fontSize="10" fill="#64748b">
					Revenue
				</text>
				<line
					x1="80"
					...
					strokeLinecap="round"
				/>
				<text x="100" y="9" fontSize="10" fill="#64748b">
					Expenses
				</text>
			</g>
		</svg>
